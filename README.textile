...
Note that the hostname & pid is added to the beginning of each line of a multi-line log statement. So even if you call logger.error() with a string that has newlines in it, the host & pid will be added to each line, not only the first. 
...
The above code will log "Oops we caught an exception", and then log the stack trace. 
