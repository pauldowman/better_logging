...
Note that the hostname & pid is added to the beginning of each line of a multi-line log statement. So even if you call logger.error() with a string that has newlines in it, the host & pid will be added to each line, not only the first. This is so that you can filter your log with a regular expression like /^igserver-7\.27471/ and not miss lines.


h2. Improved warn() and error() methods

The warn() and error() methods now allow an Exception object to be given as an optional second parameter. If the exception is given, then the stack trace will be automatically printed.
...
<pre>
  begin
    ...
    log.error "Oops we caught an exception", e
  end
</pre>

The above code will print "Oops we caught an exception", and then print the stack trace. 
...
Please let me know if you find any bugs, my contact info is here: "http://pauldowman.com/contact/":http://pauldowman.com/contact/
