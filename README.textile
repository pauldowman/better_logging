...
It adds severity level (with colour, if ActiveRecord::Base.colorize_logging == true), hostname and process ID to each log line, and it and adds warn() and exception() methods that take an exception as an argument and print the stack trace.
...
NOTE: it uses class variables to store the PID, I'm not sure, but I suspect that might be a problem with Passenger "smart spawning". Please let me know if you can confirm or deny this.
